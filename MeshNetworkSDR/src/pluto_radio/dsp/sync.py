        ...
        if metric[cursor] >= threshold:
            ...
            if sync_pos != -1 and sync_pos <= 6 and len(head_bytes) >= sync_pos + 6:
                ...
                if length <= 1500:
                    ...
                    cursor = peak_idx + pre_len + total_bits * samples_per_symbol
                else:
            ...
            sig_pwr = np.mean(np.abs(burst_corrected[:pre_len]) ** 2)
            noise_est = np.var(burst_corrected[:pre_len] - pre_syms * np.mean(np.abs(burst_corrected[:pre_len])))
